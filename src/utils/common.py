...
if __name__=='__main__':
    '''
    path ='/home/lxy/Downloads/DataSet/VOC_dataset/VOCdevkit/VOC2008/SegmentationClass/2007_000033.png'
    img = cv2.imread(path)
    padd = [(1,1),(1,1),(0,0)]
    scale_ = 2
    m = resize_mask(img, scale_, padd, crop=None)
    print(np.shape(m))
    cv2.imshow('test',m)
    cv2.waitKey(0)
    '''
    score = [[1,2,3,4,5],[2,10,4,6,7]]
    idx = [[4,3,2],[1,4,3]]
    test = [score,idx]
    def print_fun(x,y):
    ...
    '''
    for i in range(2):
        #a = print_fun(score[i],idx[i])
        for x in test:
            print("child",x)
            #print(x[i])
        #print(a)
    '''
    a= batch_slice([score,idx],print_fun,2)
    print(a)
