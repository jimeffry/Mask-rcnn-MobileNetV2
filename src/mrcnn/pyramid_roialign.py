...
import sys
sys.path.append('../')
from utils.common import parse_image_meta_graph
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        if config.BACKBONE in ["resnet50","resnet101"]:
            ...
            roi_level = log2_graph(tf.sqrt(h*w)/ (64.0 / tf.sqrt(image_area)))
            roi_level = tf.minimum(6,tf.maximum(1,2+tf.cast(tf.round(roi_level),tf.int32)))
            level_botom = 1
            level_top = 6
