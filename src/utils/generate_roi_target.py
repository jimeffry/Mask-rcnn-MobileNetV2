...
def build_roi_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks):
    ...
    for i in range(overlaps.shape[1]):
        gt = gt_boxes[i]
        overlaps[:, i] = process_img.compute_iou(
    ...
    keep_bg_ids = np.random.choice(bg_ids, remaining, replace=True)
    ...
    bboxes[pos_ids, roi_gt_class_ids[pos_ids]] = process_img.box_refinement(
    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = \
                np.round(cv2.resize(
                    class_mask, (gt_h, gt_w))).astype(bool)
            # Place the mini batch in the placeholder
            class_mask = placeholder
        # Pick part of the mask and resize it
        y1, x1, y2, x2 = rois[i].astype(np.int32)
        m = class_mask[y1:y2, x1:x2]
        mask = cv2.resize(m, config.MASK_SHAPE)
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks):
    ...
    gt_masks = tf.boolean_mask(gt_masks,non_zeros,axis=2,name="trim_gt_masks")
    ...
    #roi index max overlap
    roi_iou_argmax = tf.argmax(overlaps,axis=1)
    # Determine positive and negative ROIs
    roi_iou_max = tf.reduce_max(overlaps, axis=1)
    # 1. Positive ROIs are those with >= 0.5 IoU with a GT box
    positive_roi_bool = (roi_iou_max >= 0.5)
    #positive_indices = tf.where(positive_roi_bool)[:, 0]
    positive_indices = tf.boolean_mask(roi_iou_argmax,positive_roi_bool)
    # 2. Negative ROIs are those with < 0.5 with every GT box. Skip crowds.
    #negative_indices = tf.where(tf.logical_and(roi_iou_max < 0.5, no_crowd_bool))[:, 0]
    negative_roi_bool = (tf.logical_and(roi_iou_max < 0.5, no_crowd_bool))
    negative_indices = tf.boolean_mask(roi_iou_argmax,negative_roi_bool)
    ...
    positive_rois = tf.boolean_mask(proposals,positive_roi_bool)
    negative_rois = tf.boolean_mask(proposals,negative_roi_bool)
    ...
    deltas = common.box_refinement_graph(positive_rois, roi_gt_boxes)
    deltas /= config.BBOX_STD_DEV
    # Assign positive ROIs to GT masks
    # Permute masks to [N, height, width, 1]
    transposed_masks = tf.expand_dims(tf.transpose(gt_masks, [2, 0, 1]), -1)
    #transposed_masks = tf.transpose(gt_masks, [2, 0, 1])
