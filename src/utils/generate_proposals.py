...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Improve performance by training to top anchors by score and doing the rest on the smaller subset.
        pre_nms_limit = tf.minimum(config.PROPOSAL_TRAIN_PER_IMAGE, tf.shape(anchors)[1])
        idx = tf.nn.top_k(scores, pre_nms_limit, sorted=True,
        ...
        boxes = common.batch_slice([pre_nms_anchors, deltas],
                                  lambda x, y: apply_box_deltas_graph(x, y),
                                  config.IMAGES_PER_GPU,
                                  names=["refined_anchors"])

        # Clip to image boundaries. Since we're in normalized coordinates,clip to 0..1 range. [batch, N, (y1, x1, y2, x2)]
