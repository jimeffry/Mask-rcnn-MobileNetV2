...
from utils.generate_roi_target import DetectionTargetLayer
from data.data_generater import data_generator
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = KL.Lambda(lambda x: UT.norm_boxes_graph(x, K.shape(input_image)[1:3]))(input_gt_boxes)
        ...
        if config.BACKBONE in ["resnet50","resnet101"]:
            _, C2, C3, C4, C5 = get_symbol(input_image, config.BACKBONE,train_bn=config.TRAIN_BN)
        else:
            C1,C2, C3, C4, C5 = get_symbol(input_image, config.BACKBONE,train_bn=config.TRAIN_BN)
        # Top-down Layers
        # TODO: add assert to varify feature map sizes match what's in config
        # P6 is used for the 5th anchor scale in RPN. Generated by subsampling from P5 with stride of 2.
        if config.BACKBONE in ["resnet50","resnet101"]:
            P2, P3, P4, P5 = get_fpn([C2, C3, C4, C5])
            P6 = KL.MaxPooling2D(pool_size=(1, 1), strides=2, name="fpn_p6")(P5)
        # Note that P6 is used in RPN, but not in the classifier heads.
            rpn_feature_maps = [P2, P3, P4, P5, P6]
            mrcnn_feature_maps = [P2, P3, P4, P5]
        else:
            P1,P2, P3, P4, P5 = get_fpn([C1,C2,C3,C4,C5])
            rpn_feature_maps = [P1,P2,P3,P4,P5]
        ...
        # RPN Model
        rpn_net = build_rpn_model(config.RPN_ANCHOR_STRIDE,len(config.RPN_ANCHOR_RATIOS), config.TOP_DOWN_PYRAMID_SIZE)
        # Loop through pyramid layers
        layer_outputs = []  # list of lists
        for p in rpn_feature_maps:
            #rpn_out = rpn_graph(p, len(config.RPN_ANCHOR_RATIOS), config.RPN_ANCHOR_STRIDE)
            rpn_out = rpn_net(p)
            layer_outputs.append(rpn_out)
        ...
        rpn_rois = ProposalLayer(proposal_count=proposal_count,nms_threshold=config.RPN_NMS_THRESHOLD,name="ROI")([rpn_class, rpn_bbox, anchors])
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                # Normalize coordinates
                target_rois = KL.Lambda(lambda x: UT.norm_boxes_graph(
                    x, K.shape(input_image)[1:3]))(input_rois)
            else:
                target_rois = rpn_rois
            # Generate detection targets
            # Subsamples proposals and generates target outputs for training
            # Note that proposal class IDs, gt_boxes, and gt_masks are zeropadded.
            # Equally, returned rois and targets are zero padded.
            rois, target_class_ids, target_bbox, target_mask =\
                DetectionTargetLayer(name="proposal_targets")([target_rois, input_gt_class_ids, gt_boxes, input_gt_masks])
            # Network Heads
            ...
            print("rpn shape",K.int_shape(rois))
            output_rois = tf.identity(tf.expand_dims(rois,0),name="output_rois")
            # Losses
            ...
            # Model
            inputs_data = [input_image, input_image_meta,
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            if layer.output in self.keras_model.losses:
                print("pass loss,layer output: ",layer.output,self.keras_model.losses)
                continue
            loss = (
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        for layer in layers:
            ...
            if layer.__class__.__name__ == 'Model':
                print("child  model: ", layer.name)
                self.set_trainable(
                    layer_regex, keras_model=layer, indent=indent + 4)
                continue
            if not layer.weights:
                #print("pass,layer no weights: ",layer.name)
                continue
            # Is it trainable?
            trainable = bool(re.match(layer_regex, layer.name))
            # Update layer. If layer is a container, update inner layer.
            if layer.__class__.__name__ == 'TimeDistributed':
                if layer.layer.__class__.__name__ == 'BatchNormalization':
                    trainable = False
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs,patience,
        ...
        train_image_total_num = len(train_dataset.image_info)
        train_batch_num = np.ceil(train_image_total_num/np.float(config.BATCH_SIZE))
        val_image_total_num = len(val_dataset.image_info)
        val_batch_num = np.ceil(val_image_total_num/np.float(config.BATCH_SIZE))
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding, crop = UT.resize_image(
                image,
                min_dim=self.config.IMAGE_MIN_DIM,
                min_scale=self.config.IMAGE_MIN_SCALE,
                max_dim=self.config.IMAGE_MAX_DIM,
                mode=self.config.IMAGE_RESIZE_MODE)
            molded_image = UT.mold_image(molded_image)
            # Build image_meta
            image_meta = UT.compose_image_meta(
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        #print("window in detect: ",window)
        window = UT.norm_boxes(window, image_shape[:2])
        wy1, wx1, wy2, wx2 = window
        #print("norm window: %f " % window[0])
        ...
        #show_scale = scale
        #print("scales in model.py: ",map(show_scale,str))
        # Convert boxes to pixel coordinates on the original image
        boxes = UT.denorm_boxes(boxes, original_image_shape[:2])
        ...
        full_masks = []
        #print("detect box : ",boxes)
        for i in range(N):
            # Convert neural network mask to full size mask
            full_mask = UT.unmold_mask(masks[i], boxes[i], original_image_shape)
